...
parser.add_argument('map', nargs='?', default='flat')
parser.add_argument('center', nargs='?', help='Use format: [x,y,z]')
parser.add_argument('boundary_size', nargs='?', help='Use format: [h,v]')
parser.add_argument('zoom', nargs='?', default='0')
...
if args.world and args.map and args.center and args.boundary_size and args.dest and args.zoom:
    ...
    if args.map not in maps.keys():
        logging.error('map not found, use: main.py http://dynmap-address world_name map_name center width_x_height_tiles')
        for name in maps.keys():
            print('%s - %s' % (name, maps[name].title))
        sys.exit(1)
